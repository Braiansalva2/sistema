...
<div class="container-fluid py-3">
    ...
    <div class="row mb-4 align-items-center">
        <div class="col">
            <h2 class="fw-bold mb-0">Editar Tramo</h2>
            <small class="text-muted">Modificar configuración de rutas y parámetros comerciales</small>
        </div>
    ...
    <form method="POST" action="{{ route('comercial.tramos.update', $tramo->id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="card shadow-sm border-0">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="origen_id" id="origen_id" class="form-select select2" required>
                            ...
                            @foreach($ubicaciones as $u)
                                <option value="{{ $u->id }}"
                                    {{ old('origen_id', $tramo->origen_id) == $u->id ? 'selected' : '' }}>
                                    {{ $u->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="destino_id" id="destino_id" class="form-select select2" required>
                            ...
                            @foreach($ubicaciones as $u)
                                <option value="{{ $u->id }}"
                                    {{ old('destino_id', $tramo->destino_id) == $u->id ? 'selected' : '' }}>
                                    {{ $u->nombre }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="tipos_vehiculos_id" class="form-select">
                            <option value="">Seleccione</option>
                            @foreach($tiposVehiculos as $vehiculo)
                                <option value="{{ $vehiculo->id }}"
                                    {{ old('tipos_vehiculos_id', $tramo->tipos_vehiculos_id) == $vehiculo->id ? 'selected' : '' }}>
                                    {{ $vehiculo->nombre }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="tipo_servicio_id" id="tipo_servicio_id" class="form-select" required>
                            ...
                            @foreach($tiposServicio as $tipo)
                                <option value="{{ $tipo->id }}"
                                    {{ old('tipo_servicio_id', $tramo->tipos_servicio_id) == $tipo->id ? 'selected' : '' }}>
                                    {{$tipo->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number"
                               step="0.01"
                               name="distancia_km"
                               value="{{ old('distancia_km', $tramo->distancia_km) }}"
                               class="form-control">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number"
                               ...
                               value="{{ old('tiempo_estimado_min', $tramo->tiempo_estimado_min) }}"
                               class="form-control">
                    </div>
            ...
            <div class="card-body">
                <textarea name="observaciones"
                          rows="3"
                          class="form-control"
                          placeholder="Detalles adicionales...">{{ old('observaciones', $tramo->observaciones) }}</textarea>
            </div>
            ...
            <div class="card-body">
                <select name="estado" class="form-select">
                    <option value="activo" {{ old('estado', $tramo->estado) == 'activo' ? 'selected' : '' }}>
                        Activo
                    </option>
                    <option value="inactivo" {{ old('estado', $tramo->estado) == 'inactivo' ? 'selected' : '' }}>
                        Inactivo
                    </option>
                </select>
            </div>
            ...
            <div class="card-body">
                ...
                <div id="contenedor-tarifas">
                    ...
                    @php
                        $tarifasOld = old('tarifas');
                        $tarifasMostrar = $tarifasOld ?? $tramo->tarifas;
                    @endphp

                    @forelse($tarifasMostrar as $i => $tarifa)

                        <div class="row g-3 mb-3 tarifa-item">

                            @if(!is_array($tarifa))
                                <input type="hidden" name="tarifas[{{ $i }}][id]" value="{{ $tarifa->id }}">
                            @elseif(isset($tarifa['id']))
                                <input type="hidden" name="tarifas[{{ $i }}][id]" value="{{ $tarifa['id'] }}">
                            @endif
                            ...
                            <div class="col-md-2">
                                ...
                                <select name="tarifas[{{ $i }}][tipo]" class="form-select">
                                    @php
                                        $tipoValue = is_array($tarifa) ? ($tarifa['tipo'] ?? '') : $tarifa->tipo;
                                    @endphp
                                    <option value="">Seleccione</option>
                                    <option value="viaje" {{ $tipoValue == 'viaje' ? 'selected' : '' }}>Viaje</option>
                                    <option value="km" {{ $tipoValue == 'km' ? 'selected' : '' }}>Km</option>
                                    <option value="m3" {{ $tipoValue == 'm3' ? 'selected' : '' }}>m³</option>
                                    <option value="tonelada" {{ $tipoValue == 'tonelada' ? 'selected' : '' }}>Tonelada</option>
                                    <option value="hora" {{ $tipoValue == 'hora' ? 'selected' : '' }}>Hora</option>
                                </select>
                            </div>
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="number"
                                       step="0.01"
                                       name="tarifas[{{ $i }}][precio]"
                                       class="form-control"
                                       value="{{ is_array($tarifa) ? ($tarifa['precio'] ?? '') : $tarifa->precio }}">
                            </div>
                            ...
                            <input type="date"
                                class="form-control"
                                value="{{ is_array($tarifa) 
                                    ? ($tarifa['fecha_desde'] ?? '') 
                                    : \Carbon\Carbon::parse($tarifa->fecha_desde)->format('Y-m-d') }}"
                                disabled>

                            <input type="hidden"
                                name="tarifas[{{ $i }}][fecha_desde]"
                                value="{{ is_array($tarifa) 
                                    ? ($tarifa['fecha_desde'] ?? '') 
                                    : \Carbon\Carbon::parse($tarifa->fecha_desde)->format('Y-m-d') }}">
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="date"
                                       name="tarifas[{{ $i }}][fecha_hasta]"
                                       class="form-control"
                                       value="{{ is_array($tarifa) ? ($tarifa['fecha_hasta'] ?? '') : $tarifa->fecha_hasta }}">
                            </div>
...
@push('scripts')
<script>
let index = {{ is_countable($tarifasMostrar) ? count($tarifasMostrar) : 0 }};
...
function guardarServicio() {
    ...
    .then(data => {
        ...
        form.reset();
        document.getElementById('errorServicio').innerText = "";
    })
    .catch(() => {
...
function guardarUbicacion() {
    ...
    .then(data => {
        ...
        $('#destino_id').trigger('change');

        let modal = bootstrap.Modal.getInstance(document.getElementById('modalUbicacion'));
        modal.hide();
